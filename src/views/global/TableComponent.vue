                ...
                <tr v-for="(item, i) in items" :key="i">
                    ...
                    <td v-for="(head, j) in headers" :key="j" class="text-center">
                        <slot :name="`cel-${head.value}`" :data="item">
                            ...
                            <VChip v-if="head.value == 'operationStatus.name'" small
                                :color="db.statusColor[statusText(getValue(item, head.value))]" class="font-weight-medium">
                            ...
                            <div v-else-if="optionsHabilit && head.value == 'actions'" class="d-flex" style="gap: 8px">
                                ...
                                <VBtn v-if="iconUpdate" :title="'Update'" color="transparent" size="x-small" elevation="0"
                                    icon>
                                    <VIcon @click="$emit('update', item)" size="24" icon="mdi-pencil-outline" />
                                </VBtn>
                                <VBtn v-if="iconDelete" :title="'Delete'" color="transparent" size="x-small" elevation="0"
                                    icon>
                                    <VIcon @click="$emit('delete', item.id)" size="24"
                                        :icon="item.desserts.status ? 'mdi-delete' : 'mdi-delete-restore'" />
                                </VBtn>
                                <VBtn v-if="iconVoucher && item.operationStatus.name == 'EN ESPERA DE COMPROBANTE'"
                                    :title="'Upload voucher'" color="transparent" size="x-small" elevation="0" icon>
